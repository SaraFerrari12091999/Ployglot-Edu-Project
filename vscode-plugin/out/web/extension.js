    ...
    function activate(context) {
        ...
        vscode.commands.registerCommand('extension.page2', () => {
            ...
            panel.webview.onDidReceiveMessage(message => {
				...
				let IdPath = message.Ctx;
				console.log(IdPath);			

				const apiUrlRun = 'https://polyglot-api-staging.polyglot-edu.com/api/flows/CTX/run';
				const apiUrlRunWithId = apiUrlRun.replace('CTX', IdPath);
				//console.log(apiUrlRunWithId);
				
				//open notebook for the coding exercise// lancio il download del notebook con rememeberLink

				const externalPageUrl = vscode.Uri.parse(apiUrlRunWithId);
	...
	<script type="module">
		...
		(function(){
			...
			fetch(apiUrlFirst,requestOptionsFirst)
				...
				//console.log("prima del button click");
